...
function renderCompare(){
  ...
  const summaryHtml=`<div class="card" style="margin-bottom:16px">
    <div style="font-family:'Oswald';font-weight:600;font-size:.78rem;letter-spacing:.08em;text-transform:uppercase;color:var(--muted);margin-bottom:12px">${fmtDate(a)} → ${fmtDate(b)} · ${days} days</div>
    <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:10px;text-align:center">
      ${[['Weight','weight','kg',true],['Waist','waist','″',true],['Tummy','tummy','″',true],['Chest','chest','″',false],['Bicep','bicep','″',false]].map(([lbl,k,u,lg])=>`
        <div>
          <div style="font-size:.68rem;color:var(--muted);font-family:'Oswald';letter-spacing:.04em;text-transform:uppercase;margin-bottom:4px">${lbl}</div>
  ...
  const angleHtml=CI_ANGLES.map(an=>{
    ...
    const mBadge=mLabel&&md!=null?`<div style="text-align:center;margin-bottom:8px;font-size:.8rem"><span style="font-family:'Oswald';font-weight:600;color:var(--muted)">${mLabel}: </span><span style="color:${mCol};font-weight:700">${A[mk]}″ → ${B[mk]}″ (${md>0?'+':''}${md.toFixed(1)}″)</span></div>`:'';
    return `<div style="margin-top:18px" id="cmpAngle_${an.k}">
      <div style="font-family:'Oswald';font-weight:600;font-size:.8rem;letter-spacing:.1em;text-transform:uppercase;color:var(--ink2);margin-bottom:6px">${an.label}</div>
      ${mBadge}
